...
export class ProjectChangesInfo implements IProjectChangesInfo {
	...
	constructor(platform: string,
		...
		if (force || !this.$fs.exists(buildInfoFile)) {
			...
			this.prepareInfo = this.$fs.readJson(buildInfoFile);
		...
		if (_.startsWith(file, "node_modules")) {
			...
			while(filePath !== "node_modules") {
				...
				if (fs.exists(fullFilePath)) {
					let json = fs.readJson(fullFilePath);
	...
	static getLatestPrepareInfo(platformData: IPlatformData, fs: IFileSystem): IPrepareInfo {
		...
		if (fs.exists(prepareInfoFile)) {
			let prepareInfo = fs.readJson(prepareInfoFile);
			return prepareInfo;
		}
		return null;
	}
